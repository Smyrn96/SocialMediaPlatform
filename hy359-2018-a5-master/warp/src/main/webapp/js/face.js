...
function showRadioPhotoButton(){
  document.getElementById("radioPhoto").style.visibility="visible";
}
...
function setUserID(temp_data){
  ...
  data.append('user_id',temp_username);
  data.append('face_token',temp_data.faces[0].face_token);
  userFaceToken=temp_data.faces[0].face_token;
  ajaxRequest('POST',faceAPI.setuserId,data,addUserFace);
}

function addUserFace(){
  data.append('api_key',faceAPI.api_key);
  data.append('api_secret',faceAPI.apiSecret);
  data.append('outer_id',faceAPI.app);
  data.append('face_tokens',userFaceToken);
  ajaxRequest('POST',faceAPI.addFace,data,getDetails);
}

function printInfo(){
  console.log("User's ID and face have been added!");
}
...
function getDetails(){
  ...
  data.append('api_secret',faceAPI.apiSecret);
  data.append('outer_id',faceAPI.app);
  ajaxRequest('POST',faceAPI.getDetail,data,printInfo);
}
...
function searchPhoto(){
  ...
  data.append('outer_id', faceAPI.app);
  ajaxRequest('POST',faceAPI.search,data,autoComplete);
}

function autoComplete(temp_data){
  document.getElementById('username').value=temp_data.results[0].user_id;
  console.log("User: "+temp_data.results[0].user_id+" found!");
}
